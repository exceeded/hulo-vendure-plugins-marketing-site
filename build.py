...
PLUGINS = [
    ...
    },
    {
        ...
        'features': [
            ...
            ('Manual review queue', 'Held orders wait for a human. Approve releases fulfilment and emails the customer; reject cancels and notifies. Alerts fan out to Slack, Discord, Microsoft Teams, Telegram and an HMAC-signed webhook, and a per-channel auto-approve timer means nothing strands over a weekend. Notes + full audit trail on every decision.'),
            ...
            ('Customer messages in your voice', 'Every gating outcome — held, approved, rejected — is a per-channel editable template with variables, live preview and thoughtful defaults: a held order reads as \'a quick security check\' with a stated turnaround, never an accusation, and rejections carry a refund timeline plus a human-appeal path. You choose when held customers are told: never, block-level only, or always.'),
